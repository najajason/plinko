...
$(function(){
    ...
    $.getJSON("https://api.moneypot.com/v1/list-bets?app_id="+config.app_id, function(json){
        var data = json;
        for(var i=0; i<data.length; i++){
            betsAmount++;
            
            var real_outcome = 0;
            for(var i=0; i<data[i].payouts.length; i++){
                if(data[i].outcome < data[i].payouts[i].to && data[i].outcome >= data[i].payouts[i].from){
                    real_outcome = i;
                    break;
                }
            }
            ...
            cell3.innerHTML = data[i].wager.formatMoney(0, '.', ',')+" Bits";
            cell4.innerHTML = multiplier+"x";
            cell5.innerHTML = (win?"+":"")+data[i].profit.formatMoney(2, '.', ',');
