...
function correctCaptcha(response) {
    $.ajax({
        ...
        //terminal.echo((data.amount/100)+" has been added to your balance!");
        $.get( "https://api.moneypot.com/v1/auth?access_token="+getURLParameter('access_token'), function( data ) {
            if(typeof data.user.uname !== "undefined"){
                $('#balance').text((data.user.balance).formatMoney(2,'.',','));
            }
        });
        ...
        if(error == "FAUCET_ALREADY_CLAIMED"){
            //terminal.error("Faucet already claimed");
            grecaptcha.reset();
        }else if(error == "INVALID_INPUT_RESPONSE"){
            //terminal.error("Google has rejected the response. Try to refresh and do again.");
            grecaptcha.reset();
        }
        $("#faucetClaimCaptcha").css("top", "-90px");
    });
