...
$(function(){
    ...
    $.getJSON("https://api.moneypot.com/v1/list-bets?app_id="+config.app_id+"&access_token="+config.access_token, function(json){
        ...
        for(var i=data.length-1; i>=0; i--){
            ...
            var multiplier = 0;
            if(real_outcome == 0) multiplier = t0;
            if(real_outcome == 1) multiplier = t1;
            if(real_outcome == 2) multiplier = t2;
            if(real_outcome == 3) multiplier = t3;
            if(real_outcome == 4) multiplier = t4;
            if(real_outcome == 5) multiplier = t5
            if(real_outcome == 6) multiplier = t6;
            if(real_outcome == 7) multiplier = t7;
            if(real_outcome == 8) multiplier = t8;
            if(real_outcome == 9) multiplier = t9;
            if(real_outcome == 10) multiplier = t10;
            if(real_outcome == 11) multiplier = t11;
            if(real_outcome == 12) multiplier = t12;
            if(real_outcome == 13) multiplier = t13;
            if(real_outcome == 14) multiplier = t14;
...
$('#doBetButton').click(function() {
    ...
    $.ajax({
        ...
        data: JSON.stringify({
            ...
            hash: String(Hash),
            wager: bet,
            pay_table: [t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13,t14]
        }),
        dataType: "json"
    }).done(function(data){
        if(data.next_hash){
